...
def black_scholes(request):
    ...
    if request.method=="POST":
        ...
        u=math.exp(float(sigma)*math.sqrt(float(timeperiod)/float(binomial)))
        d=math.exp(-float(sigma)*math.sqrt(float(timeperiod)/float(binomial)))
        pu=((math.exp(float(rate)*float(timeperiod)/float(binomial)))-float(d))/(float(u)-float(d))
        pd=1-pu
        disc=math.exp(-float(rate)*float(timeperiod)/float(binomial))

        St = [0] * (int(binomial)+1)
        C = [0] * (int(binomial)+1)
    
        St[0]=float(current)*float(d)**int(binomial)
        ...
        ob.update({"type":type})
        ob.update({"rate":rate})
        ob.update({"upfactor":u})
        ob.update({"downfactor":d})
        ob.update({"timeperiod":timeperiod})
        ob.update({"strike":strike})
        ob.update({"current":current})
